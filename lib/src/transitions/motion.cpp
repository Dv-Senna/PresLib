#include <iostream>
#include <map>

#include <glm/gtc/matrix_transform.hpp>

#include "transitions/motion.hpp"
#include "utils/function.hpp"



namespace pl::transitions
{
	SlideTogether::SlideTogether(const pl::Transition::CreateInfo &createInfo, const glm::vec2 &viewportSize) :
		pl::Transition(createInfo, viewportSize),
		m_specificCreateInfo {}
	{
		if (!m_createInfo.data.has_value())
			return;

		if (m_createInfo.data.type() != typeid(pl::transitions::SlideTogether::CreateInfo))
			throw std::invalid_argument("PL : Invalid create info given to SlideTogether transition");

		m_specificCreateInfo = std::any_cast<pl::transitions::SlideTogether::CreateInfo> (m_createInfo.data);
	}



	void SlideTogether::launch()
	{
		pl::Transition::launch();
	}



	void SlideTogether::run(
		pl::utils::Millisecond dt,
		glm::mat4 &oldSlide,
		glm::mat4 &nextSlide,
		pl::utils::Color & /* oldColor */,
		pl::utils::Color & /* nextColor */
	)
	{
		static std::map<pl::utils::Axis, glm::vec3> axisMaps {
			{pl::utils::Axis::x, glm::vec3(1.f, 0.f, 0.f)},
			{pl::utils::Axis::rx, glm::vec3(-1.f, 0.f, 0.f)},
			{pl::utils::Axis::y, glm::vec3(0.f, 1.f, 0.f)},
			{pl::utils::Axis::rx, glm::vec3(0.f,-10.f, 0.f)},
		};

		m_totalRunTime += dt;
		if (m_totalRunTime < m_createInfo.delay * 1000.f)
			return;

		auto moveOldFar {glm::translate(glm::mat4(1.f), glm::vec3(0.f, 0.f, 0.1f))};
		glm::vec3 axis {axisMaps[m_specificCreateInfo.axis]};
		float scaleFactor {m_viewportSize.x};
		if (m_specificCreateInfo.axis == pl::utils::Axis::y || m_specificCreateInfo.axis == pl::utils::Axis::ry)
			scaleFactor = m_viewportSize.y;

		oldSlide = pl::utils::linear(
			(m_totalRunTime - m_createInfo.delay * 1000.f) / m_createInfo.duration / 1000.f,
			moveOldFar,
			glm::translate(glm::mat4(1.f), (2.f + m_specificCreateInfo.spaceBetweenSlides / scaleFactor) * axis)
		);
		nextSlide = pl::utils::linear(
			(m_totalRunTime - m_createInfo.delay * 1000.f) / m_createInfo.duration / 1000.f,
			glm::translate(glm::mat4(1.f), (-2.f - m_specificCreateInfo.spaceBetweenSlides / scaleFactor) * axis),
			glm::mat4(1.f)
		);

		if (m_totalRunTime >= m_createInfo.duration * 1000.f + m_createInfo.delay * 1000.f)
		{
			m_isRunning = false;
			nextSlide = glm::mat4(1.f);
		}
	}



} // namespace pl::transitions
